new-item ${env:APPDATA}/'Moonchild Productions'/'Pale Moon'/Profiles -itemtype directory 
& ${env:ProgramFiles(x86)}/WinRAR/winrar.exe x -ibck ${env:USERPROFILE}/Desktop/imacros/Profiles.rar *.* ${env:APPDATA}/'Moonchild Productions'/'Pale Moon'/Profiles
...
for ($i = [int]$f; $i -le [int]$l; $i++){
  $temp = $run -replace '@ip',"$p`_$i"
  $temp = $temp -replace '@pass',$pass
  $path = 'C:\Logs\Imacros\' + "$p`_$i.js"
  $temp | Out-File $path
  & ${env:ProgramFiles(x86)}/'Pale Moon'/palemoon.exe -p $p`_$i -no-remote imacros://run/?m=$p`_$i.js
}
